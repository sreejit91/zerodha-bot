...
HORIZON      = 5    # lookahead bars
THR_ATR_MULT = 1.0  # threshold = atr/close * this

def add_labels(df: pd.DataFrame,
               horizon: int = HORIZON,
               thr_atr_mult: float = THR_ATR_MULT) -> pd.DataFrame:
    """
    Threshold‑Direction labeling:
      • raw future return over next `horizon` bars
      • dynamic threshold = atr/close * thr_atr_mult
      • label = 1 if future_ret>threshold; 0 if future_ret<–threshold; otherwise NaN
      • drop all NaN‐labeled rows
    """
    df = df.copy()
    if "atr" not in df:
    ...
    # 3) assign label or NaN
    df["label"] = np.where(df["future_ret"] >  thr, 1,
                   np.where(df["future_ret"] < -thr, 0, np.nan))
    # 4) drop everything without a label
